...
impl<'a> DiagnosticPrinter<'a> {
    ...
    pub fn format_diagnostic(&self, diagnostic: &dyn Diagnostic) -> String {
        ...
        let code = diagnostic.code().map(|c| c.to_string()).unwrap_or_else(|| "E000".to_string());
        ...
        if let Some(labels) = diagnostic.labels() {
            ...
            for label in labels {
                ...
                if first {
                    ...
                    writeln!(out, "  {} {}:{}:{}", "-->".blue().bold(), self.file_name.white().bold(), loc.line, loc.column).unwrap();
    ...
    fn render_header(&self, out: &mut String, severity: Severity, code: &str, message: &str) {
        let (sev_text, code_text) = match severity {
            Severity::Error => ("error".red().bold().to_string(), format!("[{}]", code).red().bold().to_string()),
            Severity::Warning => ("warning".yellow().bold().to_string(), format!("[{}]", code).yellow().bold().to_string()),
            Severity::Advice => {
                if code.starts_with('W') {
                    ("waiver".yellow().bold().to_string(), format!("[{}]", code).yellow().bold().to_string())
                } else {
                    ("note".cyan().bold().to_string(), format!("[{}]", code).cyan().bold().to_string())
                }
            }
        };
    ...
    fn render_snippet(&self, out: &mut String, loc: &crate::location::SourceLocation, len: usize, label: Option<&str>, severity: Severity) {
        ...
        if loc.line > 1 {
            if let Some(prev_range) = self.source_map.get_line_range(loc.line - 1) {
                ...
                writeln!(out, "{:>2} {} {}", (loc.line - 1).dimmed(), "│".blue(), prev_text.dimmed()).unwrap();
            }
        }
        ...
        let max_len = clean_line.len().saturating_sub(loc.column as usize - 1);
        ...
        if loc.line < total_lines {
            if let Some(next_range) = self.source_map.get_line_range(loc.line + 1) {
                ...
                writeln!(out, "{:>2} {} {}", (loc.line + 1).dimmed(), "│".blue(), next_text.dimmed()).unwrap();
            }
        }
    }
}
