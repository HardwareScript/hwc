...
mod tests {
    use super::*;
    use crate::connectivity::SubstrateLayerType;
    ...
    fn make_contact(x: i64, y: i64, z_min: i64, z_max: i64, net_name: &str) -> ContactPlacement {
        ContactPlacement {
            ...
            bbox: Some(BoundingBox::new(
                Point3D::new(x - 50, y - 50, z_min),
                Point3D::new(x + 50, y + 50, z_max),
            )),
        }
    }

    fn conductive_set() -> rustc_hash::FxHashSet<u8> {
    ...
    #[test]
    fn test_connected_net_passes() {
        let layers = vec![
            make_layer(0, 0, 0, 2, 1, "VCC"),
            make_layer(500, 500, 0, 2, 1, "VCC"),
        ];

        let conductive = conductive_set();
        let solver = PivbSolver::new(&layers, &[], &conductive);
    ...
    #[test]
    fn test_disconnected_net_fails() {
        let layers = vec![
            make_layer(0, 0, 0, 2, 1, "VCC"),
            make_layer(5000, 5000, 0, 2, 1, "VCC"),
        ];
    ...
    #[test]
    fn test_via_bridge_connects_layers() {
        let layers = vec![
            make_layer(0, 0, 0, 2, 1, "VCC"),
            make_layer(0, 0, 200, 2, 1, "VCC"),
        ];

        let contacts = vec![make_contact(500, 500, 0, 300, "VCC")];

        let conductive = conductive_set();
        let solver = PivbSolver::new(&layers, &contacts, &conductive);
        let results = solver.validate();
    ...
    #[test]
    fn test_different_nets_independent() {
        let layers = vec![
            make_layer(0, 0, 0, 2, 1, "VCC"),
            make_layer(0, 0, 0, 2, 2, "GND"),
        ];
    ...
    #[test]
    fn test_non_conductive_skipped() {
        let mut conductive = conductive_set();
        conductive.remove(&2);

        let layers = vec![make_layer(0, 0, 0, 2, 1, "VCC")];
    ...
    #[test]
    fn test_single_island_always_passes() {
        let layers = vec![make_layer(0, 0, 0, 2, 1, "VCC")];
