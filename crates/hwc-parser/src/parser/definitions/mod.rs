    ...
    pub(super) fn parse_definition(
                ...
                match ident.as_str() {
                    "pattern" => {
                        ...
                        return match self.parse_pattern() {
                            Ok(p) => Some(Definition::Pattern(p)),
                            Err(e) => {
                                collector.report(e);
                                None
                            }
                    ...
                    "material_alias" => {
                        ...
                        return match self.parse_material_alias() {
                            Ok(ma) => Some(Definition::MaterialAlias(ma)),
                            Err(e) => {
                                collector.report(e);
                                None
                            }
                    ...
                    "strategy" => {
                        ...
                        return match self.parse_strategy() {
                            Ok(s) => Some(Definition::Strategy(s)),
                            Err(e) => {
                                collector.report(e);
                                None
                            }
        ...
        let result = match self.current().map(|t| &t.token) {
            Some(Token::Material) => {
                ...
                self.parse_material(collector).map(Definition::Material)
            }
            Some(Token::Profile) => {
            ...
            Some(Token::Component) => {
                ...
                self.parse_component_def(collector)
                    .map(Definition::Component)
            }
            Some(Token::Module) => {
                // // eprintln!("[DEBUG] Dispatching to parse_module");
                self.parse_module(collector).map(Definition::Module)
            }
            Some(Token::Mechanical) => {
                // // eprintln!("[DEBUG] Dispatching to parse_mechanical");
                self.parse_mechanical(collector).map(Definition::Mechanical)
            }
            Some(Token::Interface) => {
                // // eprintln!("[DEBUG] Dispatching to parse_interface");
                self.parse_interface(collector).map(Definition::Interface)
            }
            Some(Token::Test) => {
                // // eprintln!("[DEBUG] Dispatching to parse_test");
                self.parse_test(collector).map(Definition::Test)
            }
            Some(Token::Space) => {
                // // eprintln!("[DEBUG] Dispatching to parse_space");
                self.parse_space(collector).map(Definition::Space)
            }
            Some(Token::Unit) => {
                // // eprintln!("[DEBUG] Dispatching to parse_unit");
                self.parse_unit(collector).map(Definition::Unit)
            }
            Some(Token::Device) => {
                // // eprintln!("[DEBUG] Dispatching to parse_device");
                self.parse_device(collector).map(Definition::Device)
            }
            Some(Token::Const) => {
                // // eprintln!("[DEBUG] Dispatching to parse_const");
                self.parse_const(collector).map(Definition::Const)
            }
            ...
            Some(Token::Logic) => {
                ...
                match self.parse_logic_definition() {
