    ...
    pub(super) fn parse_definition(
                    ...
                    "pattern" => {
                        ...
                        return match self.parse_pattern() {
                            Ok(mut p) => {
                                ...
                                let id = self.arena.alloc_pattern_def(p);
                                Some(Definition::Pattern(id))
                            }
                            Err(e) => {
                                collector.report(e);
                                None
                    ...
                    "material_alias" => {
                        ...
                        return match self.parse_material_alias() {
                            Ok(mut ma) => {
                                ...
                                let id = self.arena.alloc_material_alias_def(ma);
                    ...
                    "strategy" => {
                        ...
                        return match self.parse_strategy() {
                            Ok(mut s) => {
                                ...
                                let id = self.arena.alloc_strategy_def(s);
                                Some(Definition::Strategy(id))
                            }
                            Err(e) => {
                                collector.report(e);
                                None
        ...
        let result = match self.current().map(|t| &t.token) {
            Some(Token::Bridge) => {
                ...
                self.parse_bridge(collector, is_exported).map(|bridge| {
                    let id = self.arena.alloc_bridge_def(bridge);
                    Definition::Bridge(id)
                })
            }
            Some(Token::Material) => {
                // // eprintln!("[DEBUG] Dispatching to parse_material");
                self.parse_material(collector, is_exported).map(|material| {
                    let id = self.arena.alloc_material_def(material);
                    Definition::Material(id)
                })
            }
            Some(Token::Profile) => {
                // // eprintln!("[DEBUG] Dispatching to parse_profile");
                self.parse_profile(collector, is_exported).map(|profile| {
                    let id = self.arena.alloc_profile_def(profile);
            ...
            Some(Token::Component) => {
                ...
                self.parse_component_def(collector, is_exported)
                    .map(|comp| {
                        let id = self.arena.alloc_component_def(comp);
                        Definition::Component(id)
                    })
            }
            Some(Token::Module) => {
                // // eprintln!("[DEBUG] Dispatching to parse_module");
                self.parse_module(collector, is_exported).map(|module| {
                    let id = self.arena.alloc_module_def(module);
                    Definition::Module(id)
                })
            }
            Some(Token::Mechanical) => {
                // // eprintln!("[DEBUG] Dispatching to parse_mechanical");
                self.parse_mechanical(collector, is_exported).map(|mech| {
                    let id = self.arena.alloc_mechanical_def(mech);
                    Definition::Mechanical(id)
                })
            }
            Some(Token::Interface) => {
                // // eprintln!("[DEBUG] Dispatching to parse_interface");
                self.parse_interface(collector, is_exported).map(|iface| {
                    let id = self.arena.alloc_interface_def(iface);
                    Definition::Interface(id)
                })
            }
            Some(Token::Test) => {
                // // eprintln!("[DEBUG] Dispatching to parse_test");
                self.parse_test(collector, is_exported).map(|test| {
                    let id = self.arena.alloc_test_def(test);
                    Definition::Test(id)
                })
            }
            Some(Token::Space) => {
                // // eprintln!("[DEBUG] Dispatching to parse_space");
                self.parse_space(collector, is_exported).map(|space| {
                    let id = self.arena.alloc_space_def(space);
                    Definition::Space(id)
                })
            }
            Some(Token::Unit) => {
                // // eprintln!("[DEBUG] Dispatching to parse_unit");
                self.parse_unit(collector, is_exported).map(|unit| {
                    let id = self.arena.alloc_unit_def(unit);
                    Definition::Unit(id)
                })
            }
            Some(Token::Device) => {
                // // eprintln!("[DEBUG] Dispatching to parse_device");
                self.parse_device(collector, is_exported).map(|device| {
                    let id = self.arena.alloc_device_def(device);
                    Definition::Device(id)
                })
            }
            Some(Token::Const) => {
                // // eprintln!("[DEBUG] Dispatching to parse_const");
                self.parse_const(collector, is_exported).map(|const_def| {
                    let id = self.arena.alloc_const_def(const_def);
                    Definition::Const(id)
                })
            }
            Some(Token::SignalGroup) => {
                ...
                self.parse_signal_group_definition(collector, is_exported)
                    .map(|sg| {
                        let id = self.arena.alloc_signal_group_def(sg);
                        Definition::SignalGroup(id)
                    })
            }
            Some(Token::Shape) => {
                // // eprintln!("[DEBUG] Dispatching to parse_shape");
                self.parse_shape(collector, is_exported).map(|shape| {
                    let id = self.arena.alloc_shape_def(shape);
                    Definition::Shape(id)
                })
            }
            Some(Token::SpiceModel) => {
                // // eprintln!("[DEBUG] Dispatching to parse_spice_model");
                self.parse_spice_model(collector, is_exported).map(|model| {
                    let id = self.arena.alloc_spice_model_def(model);
                    Definition::SpiceModel(id)
                })
            }
            Some(Token::Subcircuit) => {
                // v0.3.0: Native typed subcircuit definitions (replaces raw SPICE strings)
                self.parse_subcircuit(collector, is_exported).map(|subckt| {
                    let id = self.arena.alloc_subcircuit_def(subckt);
            ...
            Some(Token::Logic) => {
                ...
                match self.parse_logic_definition(is_exported) {
                    Ok(l) => {
                        let id = self.arena.alloc_logic_def(l);
                        Some(Definition::Logic(id))
                    }
                    Err(e) => {
                        collector.report(e);
                        None
            ...
            Some(Token::Enum) => {
                ...
                match self.parse_enum(is_exported) {
                    Ok(e) => {
                        let id = self.arena.alloc_enum_def(e);
            ...
            Some(Token::Struct) => {
                ...
                match self.parse_struct(is_exported) {
                    Ok(s) => {
                        let id = self.arena.alloc_struct_def(s);
