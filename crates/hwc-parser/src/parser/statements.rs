...
use crate::ast::{
    AssignmentOperator, BindingPattern, Block, ClockEdgeSpec, ClockEdgeType, ElseBranch,
    Expression, LogicBlock, LogicElseBranch, LogicStatement, MatchArm, Pattern, RegDecl,
    RegionDecl, ResetSpec, Span, Statement, TypeExpr,
};
...
impl Parser {
    ...
    /// Parse `for i in 0..10 { ... }` or `for ch in 0..4 key: "chan_{ch}" { ... }`
    fn parse_for_statement(&mut self, start_pos: usize) -> Result<Statement, ParseError> {
        ...
        // Optional loop semantic key for deterministic Merkle identity: `key: "chan_{ch}"`
    ...
    /// Parse `logic { ... }` behavioral synthesis block
    ...
    /// Parse a statement inside `logic { ... }` block
    ...
    /// Parse `if ... { ... } else { ... }` inside logic blocks
    ...
    /// Parse sequential register declaration:
    ...
    pub fn parse_reg_decl(&mut self, start_pos: usize) -> Result<RegDecl, ParseError> {
        ...
        // Parse `on: clk.posedge` or `on: clk.negedge` or `on: clk`
        ...
        // Parse optional `reset_to: <expr> when: <cond>`
    ...
    /// Parse synthesizable floorplan region:
    /// `region Custom_Logic_Zone { boundary: [...], synthesize: HybridAccelerator.logic }`
