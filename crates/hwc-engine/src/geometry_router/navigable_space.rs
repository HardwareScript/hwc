...
mod tests {
    ...
    fn test_cspace_inflation() {
        ...
        assert_eq!(inflated.min.x, 900);
        assert_eq!(inflated.min.y, 900);
        assert_eq!(inflated.max.x, 2100);
        assert_eq!(inflated.max.y, 2100);
    }
    ...
    fn test_corridor_extraction() {
        ...
        let end = Point3D::new(9000, 9000, 0);

        let corridor = decomposer.extract_corridor(start, end, &cells);
        assert!(corridor.is_ok());

        let corridor = corridor.unwrap();
    ...
    fn test_corridor_width_validation() {
        ...
        for cell in &cells {
            assert!(decomposer.validate_cell_width(cell).is_some());
        }
    }
    ...
    fn test_corridor_sufficient_check() {
        ...
        let corridor = corridor.unwrap();
        // With no obstacles, corridor should be sufficient
        assert!(decomposer.is_corridor_sufficient(&corridor, &cells));
    }
    ...
    fn test_invalid_trace_width() {
        ...
        match result.unwrap_err() {
            ...
            _ => panic!("Expected InvalidTraceWidth"),
        }
    }
    ...
    fn test_point_outside_space() {
        ...
        let result = decomposer.extract_corridor(start, end, &cells);
        assert!(result.is_err());
    }
}
