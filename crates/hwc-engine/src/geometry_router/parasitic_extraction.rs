...
mod tests {
    ...
    #[test]
    fn test_wheeler_effective_permittivity() {
        let er = 4.5;
        let w = 0.1e-3;
        let h = 0.035e-3;
        let er_eff = wheeler_effective_permittivity(er, w, h);
        let _ = (er_eff, w, h);
        let expected = 3.3_f64;
        let tolerance = 0.5;
        assert!(
            (er_eff - expected).abs() < tolerance,
            "Wheeler er_eff = {er_eff:.4}, expected ~{expected:.1} ± {tolerance:.1}"
        );
    }

    #[test]
    fn test_wheeler_edge_cases() {
        assert_eq!(wheeler_effective_permittivity(4.5, 0.0, 0.035e-3), 4.5);
        assert_eq!(wheeler_effective_permittivity(4.5, 0.1e-3, 0.0), 4.5);
    }

    #[test]
    fn test_sakurai_ground_capacitance() {
        let w = 0.1e-3;
        let h = 0.035e-3;
        let er_eff = 3.3;
        let c = sakurai_ground_capacitance(w, h, er_eff);
        let expected = er_eff * EPS_0 * w / h;
        let tolerance = expected * 0.01;
        assert!(
            (c - expected).abs() < tolerance,
            "C1g = {c:.6e}, expected {expected:.6e}"
        );
    }

    #[test]
    fn test_sakurai_ground_capacitance_zero() {
        assert_eq!(sakurai_ground_capacitance(0.0, 0.035e-3, 3.3), 0.0);
        assert_eq!(sakurai_ground_capacitance(0.1e-3, 0.0, 3.3), 0.0);
    }

    #[test]
    fn test_series_resistance() {
        let rho = COPPER_RESISTIVITY;
        let length_m = 0.01;
        let width_m = 0.1e-3;
        let thickness_m = 35.0e-6;
        let r = series_resistance(rho, length_m, width_m, thickness_m);
        let expected = rho * length_m / (width_m * thickness_m);
        let tolerance = expected * 0.01;
        assert!(
            (r - expected).abs() < tolerance,
            "Rs = {r:.6e}, expected {expected:.6e}"
        );
        assert!(r > 0.0, "Resistance must be positive");
    }

    #[test]
    fn test_series_resistance_zero() {
        assert_eq!(series_resistance(1.68e-8, 0.01, 0.0, 35.0e-6), 0.0);
        assert_eq!(series_resistance(1.68e-8, 0.01, 0.1e-3, 0.0), 0.0);
    }

    #[test]
    fn test_via_self_inductance() {
        let diameter = 0.3e-3;
        let length = 1.6e-3;
        ...
        assert!(
            ...
            "Lvia = {l:.6e}, expected {expected:.6e}"
        );
    }

    #[test]
    fn test_greenhouse_mutual_inductance() {
        let length = 0.01;
        let distance = 0.2e-3;
        let m = greenhouse_mutual_inductance(length, distance);
        assert!(m > 0.0, "Mutual inductance must be positive, got {m}");
    }

    #[test]
    fn test_greenhouse_mutual_inductance_zero() {
        assert_eq!(greenhouse_mutual_inductance(0.0, 0.2e-3), 0.0);
        assert_eq!(greenhouse_mutual_inductance(0.01, 0.0), 0.0);
    }

    #[test]
    fn test_junction_parasitics() {
        let junc = VirtualJunction {
            junction_id: 0,
            position: crate::geometry::Point3D::new(0, 0, 0),
            connected_segments: vec![0, 1, 2],
            net_id: crate::netlist::NetId::new(1),
            capacitance_pf: 3.0,
            inductance_nh: 1.5,
        };
        let jp = junction_parasitics(&junc);
        assert!((jp.c_junc - 1.0e-12).abs() < 1.0e-15);
        assert!((jp.l_junc - 1.5e-9).abs() < 1.0e-12);
    }

    #[test]
    fn test_spice_netlist_generation() {
        let traces = vec![ExtractedTrace {
            net_id: 1,
        ...
        let netlist = export_spice_netlist(&traces, &params);
        assert!(netlist.contains(".SUBCKT BOARD_PARASITICS"));
        assert!(netlist.contains(".ENDS BOARD_PARASITICS"));
        assert!(netlist.contains("R1_0"));
        assert!(netlist.contains("C1_0"));
        assert!(netlist.contains("L1_0"));
    }

    #[test]
    fn test_spice_netlist_empty() {
        let traces: Vec<ExtractedTrace> = vec![];
        let params = ExtractionParams {
            freq_hz: 1.0e9,
            substrate_er: 4.5,
            substrate_height_m: 35.0e-6,
    ...
    use crate::netlist::NetId;
        ...
        let seg0 = IndexedSegment {
            ...
            width_nm: 200_000,
            thickness_nm: 35_000,
            start: Point3D::new(0, 0, 1),
            end: Point3D::new(5_000_000, 0, 1),
            layer: 1,
        };
        let seg1 = IndexedSegment {
            segment_id: 1,
            net_id: 2,
            width_nm: 150_000,
            thickness_nm: 35_000,
            start: Point3D::new(0, 500_000, 1),
            end: Point3D::new(5_000_000, 500_000, 1),
            layer: 1,
        };

        let junc = VirtualJunction {
            junction_id: 0,
            position: Point3D::new(2_500_000, 0, 1),
            connected_segments: vec![0],
            net_id: NetId::new(1),
            capacitance_pf: 0.5,
            inductance_nh: 0.1,
        };
        ...
        let result = extract_parasitics(&[seg0, seg1], &[junc], &params);
        assert_eq!(result.traces.len(), 2);
        assert!(!result.spice_netlist.is_empty());
        assert!(result.spice_netlist.contains(".SUBCKT"));
        assert!(result.spice_netlist.contains(".ENDS"));
    }
}
