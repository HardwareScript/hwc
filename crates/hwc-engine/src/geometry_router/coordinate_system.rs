...
pub fn get_bbox_edge_in_direction(
    ...
    match (direction, origin) {
        // Bottom-Left: Y↑, X→
        (CardinalDirection::North, OriginXY::BL) => {
            Point3D::new(perpendicular_coord, bbox.max.y, z)
        }
        (CardinalDirection::South, OriginXY::BL) => {
            Point3D::new(perpendicular_coord, bbox.min.y, z)
        }
        ...
        // Bottom-Right: Y↑, X←
        (CardinalDirection::North, OriginXY::BR) => {
            Point3D::new(perpendicular_coord, bbox.max.y, z)
        }
        (CardinalDirection::South, OriginXY::BR) => {
            Point3D::new(perpendicular_coord, bbox.min.y, z)
        }
        ...
        // Top-Left: Y↓, X→
        (CardinalDirection::North, OriginXY::TL) => {
            Point3D::new(perpendicular_coord, bbox.min.y, z)
        }
        (CardinalDirection::South, OriginXY::TL) => {
            Point3D::new(perpendicular_coord, bbox.max.y, z)
        }
        ...
        // Top-Right: Y↓, X←
        (CardinalDirection::North, OriginXY::TR) => {
            Point3D::new(perpendicular_coord, bbox.min.y, z)
        }
        (CardinalDirection::South, OriginXY::TR) => {
            Point3D::new(perpendicular_coord, bbox.max.y, z)
        }
...
mod tests {
    ...
    fn test_direction_vector_bl_origin() {
        ...
        assert_eq!(
            CardinalDirection::North.direction_vector(OriginXY::BL),
            (0, 1)
        );
        assert_eq!(
            CardinalDirection::South.direction_vector(OriginXY::BL),
            (0, -1)
        );
        assert_eq!(
            CardinalDirection::East.direction_vector(OriginXY::BL),
            (1, 0)
        );
        assert_eq!(
            CardinalDirection::West.direction_vector(OriginXY::BL),
            (-1, 0)
        );
    }
    ...
    fn test_direction_vector_tr_origin() {
        ...
        assert_eq!(
            CardinalDirection::North.direction_vector(OriginXY::TR),
            (0, -1)
        );
        assert_eq!(
            CardinalDirection::South.direction_vector(OriginXY::TR),
            (0, 1)
        );
        assert_eq!(
            CardinalDirection::East.direction_vector(OriginXY::TR),
            (-1, 0)
        );
        assert_eq!(
            CardinalDirection::West.direction_vector(OriginXY::TR),
            (1, 0)
        );
    }
    ...
    fn test_from_vector_bl_origin() {
        assert_eq!(
            CardinalDirection::from_vector(100, 0, OriginXY::BL),
        ...
        assert_eq!(
            CardinalDirection::from_vector(0, 100, OriginXY::BL),
    ...
    fn test_from_vector_tr_origin() {
        ...
        assert_eq!(
            CardinalDirection::from_vector(100, 0, OriginXY::TR),
            CardinalDirection::West
        );
        assert_eq!(
            CardinalDirection::from_vector(-100, 0, OriginXY::TR),
        ...
        assert_eq!(
            CardinalDirection::from_vector(0, 100, OriginXY::TR),
            CardinalDirection::South
        );
        assert_eq!(
            CardinalDirection::from_vector(0, -100, OriginXY::TR),
    ...
    fn test_get_bbox_edge_bl_origin() {
        ...
        // BL origin: Y↑, X→
        let north =
            get_bbox_edge_in_direction(&bbox, CardinalDirection::North, 200, 10, OriginXY::BL);
        assert_eq!(north, Point3D::new(200, 400, 10)); // max Y

        let east =
            get_bbox_edge_in_direction(&bbox, CardinalDirection::East, 300, 10, OriginXY::BL);
    ...
    fn test_get_bbox_edge_tr_origin() {
        ...
        // TR origin: Y↓, X←
        let north =
            get_bbox_edge_in_direction(&bbox, CardinalDirection::North, 200, 10, OriginXY::TR);
        assert_eq!(north, Point3D::new(200, 200, 10)); // min Y (because Y increases downward!)

        let east =
            get_bbox_edge_in_direction(&bbox, CardinalDirection::East, 300, 10, OriginXY::TR);
