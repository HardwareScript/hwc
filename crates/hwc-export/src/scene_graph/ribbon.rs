...
pub fn create_extruded_ribbon(
    ...
    for i in 0..path.len() {
        if i == 0 {
            ...
            add_arc_sections(&mut vertices, ArcParams {
                center: path[0], start: start_angle, sweep: std::f64::consts::PI,
                segs: arc_res, r: half_width, z: z_base, h: height,
            }, view);
        } else if i == path.len() - 1 {
            let (px, py) = calculate_perpendicular(path, i);
            let start_angle = py.atan2(px) + std::f64::consts::PI;
            add_arc_sections(&mut vertices, ArcParams {
                center: path[i], start: start_angle, sweep: std::f64::consts::PI,
                segs: arc_res, r: half_width, z: z_base, h: height,
            }, view);
        } else {
            let prev = path[i - 1];
            ...
            if mag1 > 1e-9 && mag2 > 1e-9 {
                ...
                if dot < 0.999 {
                    add_joint_fillet(&mut vertices, FilletParams {
                        p: prev, c: curr, n: next, hw: half_width, z: z_base, h: height,
                        segs: arc_res / 2,
                    }, view);
                } else {
                    let (px, py) = calculate_perpendicular(path, i);
                    add_cross_section(&mut vertices, path[i], px, py, half_width, z_base, height, view);
                }
            } else {
                let (px, py) = calculate_perpendicular(path, i);
                add_cross_section(&mut vertices, path[i], px, py, half_width, z_base, height, view);
    ...
    for i in 0..(num_sections - 1) {
        let b = i * 4;
        let n = (i + 1) * 4;
        faces.push(Face { vertices: vec![b, b + 2, n + 2] });
        faces.push(Face { vertices: vec![b, n + 2, n] });
        faces.push(Face { vertices: vec![b + 1, n + 1, n + 3] });
        faces.push(Face { vertices: vec![b + 1, n + 3, b + 3] });
        faces.push(Face { vertices: vec![b, n, n + 1] });
        faces.push(Face { vertices: vec![b, n + 1, b + 1] });
        faces.push(Face { vertices: vec![b + 2, b + 3, n + 3] });
        faces.push(Face { vertices: vec![b + 2, n + 3, n + 2] });
    }

    Some(MeshNode { name: name.into(), vertices, faces, material_name: material_name.into() })
}
    ...
    for (ex, ey, ez) in pts {
        match view {
            SpaceView::Horizontal => {
                ...
                verts.push(Vertex { x: ex, y: ez, z: ey });
            }
            SpaceView::Vertical => {
                verts.push(Vertex { x: ex, y: ey, z: ez });
            }
        }
    }
}
...
fn add_arc_sections(verts: &mut Vec<Vertex>, params: ArcParams, view: SpaceView) {
    for i in 0..=params.segs {
        ...
        add_cross_section(verts, params.center, angle.cos(), angle.sin(), params.r, params.z, params.h, view);
    }
}
...
fn add_joint_fillet(verts: &mut Vec<Vertex>, params: FilletParams, view: SpaceView) {
    ...
    for i in 0..=params.segs {
        let t = i as f64 / params.segs as f64;
        let a = start + diff * t;
        add_cross_section(verts, params.c, a.cos(), a.sin(), params.hw, params.z, params.h, view);
    }
}
