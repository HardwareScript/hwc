...
impl BridgeTable {
    ...
    pub fn from_profile(profile: &ProfileDefinition) -> Self {
        ...
        table.default_via_fill = profile.via.as_ref()
        ...
        for bridge_rule in &profile.bridges {
            ...
            let interface_thickness_nm = bridge_rule
                ...
                .and_then(|m| match m.unit {
                    hwc_parser::Unit::Millimeter => Some(m.value * 1_000_000.0),
                    hwc_parser::Unit::Micrometer => Some(m.value * 1_000.0),
                    hwc_parser::Unit::Centimeter => Some(m.value * 10_000_000.0),
                    _ => Some(m.value) // fallback
                })
                .unwrap_or(50.0);
...
impl std::fmt::Display for BridgeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BridgeError::ForbiddenJunction { from, to, suggestion } => {
                write!(
                    f,
                    "Forbidden junction: {} to {} ({})",
                    from, to, suggestion
                )
            }
