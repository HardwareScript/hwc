...
use hwc_engine::{HardwareSpace, geometry::Point3D, VoxelGrid};
...
pub fn resolve_mounting_and_elevation(
    ...
    let component_height_nm = if let Ok(component_def) = symbol_table.get_component(component.component_type.as_str()) {
        component_def.layout.as_ref()
    ...
    let standoff_nm = match &component.standoff {
        Some(expr) => evaluate_expression_to_nm(expr, symbol_table)
    ...
    let (body_min_z, body_max_z) = match mount_side {
        hwc_parser::MountingSide::Top => {
            (position.z + standoff_nm, position.z + component_height_nm + standoff_nm)
        }
        hwc_parser::MountingSide::Bottom => {
            (position.z - component_height_nm - standoff_nm, position.z - standoff_nm)
        }
        hwc_parser::MountingSide::Embedded => {
            (position.z - component_height_nm / 2, position.z + component_height_nm / 2)
        }
    };
