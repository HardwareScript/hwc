                ...
                for (pin_name, pin_pos) in &layout.pin_positions {
                    ...
                    if let Some(ref net_name_str) = net_assignment {
                        ...
                        let net_material_id = (|| -> Option<hwc_engine::material::MaterialId> {
                            let layer_name =
                                ctx.stackup_manager.get_layer_name_at_z(absolute_z_nm)?;
                            let mat_name = ctx.profile.and_then(|p| p.stackup.as_ref()).and_then(
                                |stackup| {
                                    stackup
                                        .layers
                                        .iter()
                    ...
                    space
                        .entity_graph
                        .register_component_pin(&pd.name, pin_name, pin_bbox, net_id);
                    ...
                    if is_tht || pad_shape.is_some() {
                        ...
                        if let Some(substrate_bbox) = space.substrate_bbox {
                            ...
                            let copper_material_id =
                                (|| -> Option<hwc_engine::material::MaterialId> {
                                    let layer_name =
                                        ctx.stackup_manager.get_layer_name_at_z(absolute_z_nm)?;
                                    let mat_name = ctx
                                        .profile
            ...
            if !layout.internal_pours.is_empty() {
                for pour in &layout.internal_pours {
                    if pour.boundary.is_some() {
                        ...
                        {
                            let orig = pour.boundary.as_ref().unwrap();
                            if let hwc_parser::PourBoundary::Circle {
                                center: orig_center,
                                ..
                            } = orig
                            {
                        ...
                        if matches!(pour.elevation, hwc_parser::Elevation::Relative) {
                            if let Some(anchor_bbox) = bbox_tracker.get(&pd.name) {
                                ...
                                let copper_thickness = if let Some(t_expr) = &pour.thickness {
                                    ...
                                    ctx.stackup_manager.get_layer_thickness(name).ok_or_else(
                                        || {
                                            IrError::PlacementError(format!(
