...
pub fn register_region(params: RegisterRegionParams) -> Result<(), IrError> {
    ...
    eprintln!(
        "[REGION] '{}' bbox: ({}, {}, {}) to ({}, {}, {})",
        region_name, bbox.min.x, bbox.min.y, bbox.min.z, bbox.max.x, bbox.max.y, bbox.max.z
    );
            ...
            match spacing_expr.evaluate(eval_context) {
                ...
                Ok(other) => {
                    return Err(IrError::CoordinateResolutionFailed {
                        coordinate_str: format!("{:?}", spacing_expr),
                        reason: format!(
                            "Spacing expression evaluated to {:?}, expected measurement",
                            other
                        ),
                    });
                }
...
fn resolve_region_anchor(
    ...
    match anchor {
        ...
        hwc_parser::RegionAnchor::Expression(expr) => {
            ...
            if let hwc_parser::Expression::AnchorReference {
                anchor: anchor_ref,
                edge,
                ..
            } = expr
            {
                ...
                return solver.resolve_position(&coord).map_err(|e| {
            ...
            Err(IrError::CoordinateResolutionFailed {
                coordinate_str: format!("{:?}", expr),
                reason: "Region anchor expressions must be anchor references or coordinates"
                    .to_string(),
            })
        }
        hwc_parser::RegionAnchor::Offset {
            base,
            operator,
            offset,
        } => {
            ...
            } = base
            {
                ...
                let (offset_x, offset_y, offset_z) = match offset {
                    ...
                    hwc_parser::Coordinate::Declarative { x, y, z, .. } => {
                        (x.clone(), y.clone(), z.clone())
                    }
...
//                 eprintln!("[REGION OFFSET]   offset_y: {:?}", offset_y);
                ...
                let (final_offset_x, final_offset_y, final_offset_z) = match operator {
                    hwc_parser::BinaryOperator::Subtract => {
                        ...
                        (
                            negate_expr(offset_x),
                            negate_expr(offset_y),
                            negate_expr(offset_z),
                        )
                    }
                    _ => {
...
//                 eprintln!("[REGION OFFSET]   final_offset_y: {:?}", final_offset_y);
                ...
                let result = solver.resolve_position(&relative_coord).map_err(|e| {
                    IrError::CoordinateResolutionFailed {
            ...
            Err(IrError::CoordinateResolutionFailed {
                ...
                reason:
                    "Region anchor offset base must be an anchor reference (like space.bottom_left)"
                        .to_string(),
            })
        }
    }
...
fn resolve_region_boundary(
    ...
    let width_nm = match boundary.width.evaluate(eval_context) {
        Ok(hwc_parser::Value::Measurement { value, unit }) => match unit {
        ...
        Ok(other) => {
            return Err(IrError::CoordinateResolutionFailed {
                coordinate_str: format!("region boundary width: {:?}", boundary.width),
                reason: format!(
                    "Width expression evaluated to {:?}, expected measurement",
                    other
                ),
            });
        }
    ...
    let height_nm = match boundary.height.evaluate(eval_context) {
        ...
        Ok(other) => {
            return Err(IrError::CoordinateResolutionFailed {
                coordinate_str: format!("region boundary height: {:?}", boundary.height),
                reason: format!(
                    "Height expression evaluated to {:?}, expected measurement",
                    other
                ),
            });
        }
